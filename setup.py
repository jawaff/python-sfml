...
extension = lambda name, files, libs,  lib_dir: Extension(
    name='sfml.' + name,
    sources= [os.path.join('src', 'sfml', name, filename) for filename in files],
    include_dirs=[os.path.join('include', 'Includes')],
    library_dirs=[lib_dir, os.path.join('extlibs', 'libs-msvc-universal', arch)] if sys.hexversion >= 0x03050000 else [],
    language='c++',
    libraries=libs,
...
if platform.system() == 'Windows':
    ...
    sfml_lib_dir     = os.path.join('extlibs', 'SFML-2.3.2', 'bin')
else:
    ...
    sfml_lib_dir     = os.path.join('extlibs', 'SFML-2.3.2', 'lib')
